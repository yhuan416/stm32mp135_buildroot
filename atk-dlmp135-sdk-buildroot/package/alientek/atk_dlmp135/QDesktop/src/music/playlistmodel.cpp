...
void playListModel::setPCM(bool on)
{
#ifndef __arm__
    return;
#endif
    QString cmd;
    if (on)
        cmd = tr("amixer -c STM32MP1DK cset name='PCM Playback Switch' 'on','on' >/dev/null 2>&1");
    else
        cmd = tr("amixer -c STM32MP1DK cset name='PCM Playback Switch' 'off','off' >/dev/null 2>&1");
